...
//       try {
//         const response = await fetch('http://your-backend-api/posts/');
//         if (!response.ok) {
...
let laod;
    ...
    const fetchPosts = async () => {
      try {
        const response = await fetch(
          "http://127.0.0.1:8000/frontend/all-posts/"
        );
        if (!response.ok) {
          throw new Error("Failed to fetch posts");
        }
  ...
  const changeLike = async (post) => {
    const response = await fetch("http://127.0.0.1:8000/frontend/like-post/"+post.id, {
      method: "PATCH",
      headers: {
  ...
  return (
    <div className="flex flex-col items-center mt-12">
      ...
      {posts.map((post) => (
        <div
          key={post.id}
          className="lg:flex-row sm:flex-col md:items-center flex p-2 space-x-7 mt-8"
        >
          <img
            src={post.image}
            alt={post.title}
